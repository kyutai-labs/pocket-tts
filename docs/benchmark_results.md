...
4. **INT8 quantization is currently broken** - torchao CUTLASS kernels compiled for SM90 (Ada/Hopper), incompatible with Blackwell (SM 12.0) and Ampere (SM 8.6)
5. **Quantization saves ~35% VRAM** (363 MB vs 564 MB) but kills performance without optimized kernels
...
- [ ] Fix torch.compile() support (state key `_orig_mod.` prefix issue)
