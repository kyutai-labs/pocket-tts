...
import pocket_tts.utils.voice_management as vm
...
voices_app = typer.Typer(help="Manage voices: list, inspect, blend, comparisons")
cli_app.add_typer(voices_app, name="voices")
...
def list_voices():
    ...
    console = Console()
    table = Table(title="Available Predefined Voices")
...
def _get_model_if_needed(
    voice_inputs: list[str], device: str, variant: str
) -> TTSModel | None:
    """Helper to load model if any input is an audio file (not predefined/safetensors)."""
    needs_model = False
    for v in voice_inputs:
        if (
            v not in PREDEFINED_VOICES
            and not v.endswith(".safetensors")
            and not v.startswith("hf://")
        ):
            # Assume audio file
            needs_model = True
            break
    ...
    if needs_model:
        logger.info("Loading model to process audio input...")
        model = TTSModel.load_model(variant)
        model.to(device)
        return model
    return None


@voices_app.command("similarity")
def check_similarity(
    voice1: str = typer.Argument(..., help="First voice (name, path, or URL)"),
    voice2: str = typer.Argument(..., help="Second voice (name, path, or URL)"),
    variant: Annotated[
        str, typer.Option(help="Model signature (only if encoding needed)")
    ] = DEFAULT_VARIANT,
    device: Annotated[str, typer.Option(help="Device to use")] = "cpu",
):
    """Compute similarity between two voices."""

    model = _get_model_if_needed([voice1, voice2], device, variant)

    try:
        t1 = vm.load_voice_tensor(voice1, model)
        t2 = vm.load_voice_tensor(voice2, model)

        sim = vm.compute_voice_similarity(t1, t2)
        print(f"Similarity between '{voice1}' and '{voice2}': {sim:.4f}")
...
@voices_app.command("blend")
def blend_voices(
    voice1: str = typer.Argument(..., help="First voice (name, path, or URL)"),
    voice2: str = typer.Argument(..., help="Second voice (name, path, or URL)"),
    alpha: float = typer.Option(
        0.5, help="Blending ratio (0.0 = voice1, 1.0 = voice2)"
    ),
    output: str = typer.Option("blended_voice.safetensors", help="Output filename"),
    variant: Annotated[
        str, typer.Option(help="Model signature (only if encoding needed)")
    ] = DEFAULT_VARIANT,
    device: Annotated[str, typer.Option(help="Device to use")] = "cpu",
    ...
    """
    Blend two voices and save the result as a new voice file.
    """
    model = _get_model_if_needed([voice1, voice2], device, variant)
    ...
    try:
        ...
        blended = vm.blend_voice_tensors(t1, t2, alpha)

        # Save to safetensors
        safetensors.torch.save_file({"audio_prompt": blended}, output)
        print(f"Blended voice saved to: {output}")

    except Exception as e:
        logger.error(f"Error blending voices: {e}")
        raise typer.Exit(code=1)
