...
use crate::ufunc::{get_ufunc, Ufunc, UfuncRegistry};
    ...
    F: Fn(&T, &T) -> T + Send + Sync,
{
    ...
    fn execute(
        ...
        if !self.supports_dtypes().contains(&input0.dtype().kind())
            || !self.supports_dtypes().contains(&input1.dtype().kind())
        {
    ...
    F: Fn(&T) -> T + Send + Sync,
{
    ...
    fn execute(
        ...
        if !self.supports_dtypes().contains(&input.dtype().kind()) {
            return Err(NumPyError::dtype_error(format!(
    ...
    F: Fn(&T, u32) -> Result<T> + Send + Sync,
{
    ...
    fn execute(
        ...
        if !self.supports_dtypes().contains(&input1.dtype().kind()) {
            return Err(NumPyError::dtype_error(format!(
...
{
    let engine = UfuncEngine::new();
    engine.execute_unary("logical_not", x)
}
